...
fn generate_validate_impl(input: &DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
    let expanded = quote! {
        impl #impl_generics domain_model::Validate for #name #ty_generics #where_clause {
            fn validate(&self) -> Result<(), domain_model::ValidationError> {
                let mut err = domain_model::ValidationError::default();
...
fn generate_length_validation(
    ...
    let rule = match (min, max) {
        (Some(min), Some(max)) => {
            quote! { domain_model::rules::min_len(#min).and(domain_model::rules::max_len(#max)) }
        },
        (Some(min), None) => {
            quote! { domain_model::rules::min_len(#min) }
        },
        (None, Some(max)) => {
            quote! { domain_model::rules::max_len(#max) }
        },
        (None, None) => {
            // No constraints - skip
            return quote! {};
    ...
    quote! {
        {
            let rule = #rule;
            if let Err(e) = domain_model::validate(#field_name_str, self.#field_name.as_str(), &rule) {
                err.extend(e);
            }
        }
    }
...
fn generate_range_validation(
    ...
    match (min, max) {
        (Some(min), Some(max)) => {
            quote! {
                {
                    let rule = domain_model::rules::range(#min, #max);
                    if let Err(e) = domain_model::validate(#field_name_str, &self.#field_name, &rule) {
                        err.extend(e);
                    }
                }
            }
            ...
            quote! {
                for (i, item) in self.#field_name.iter().enumerate() {
                    if let Err(e) = item.validate() {
                        let path = domain_model::Path::root().field(#field_name_str).index(i);
            ...
            quote! {
                {
                    ...
                    for (i, item) in self.#field_name.iter().enumerate() {
                        let path = domain_model::Path::root().field(#field_name_str).index(i);
                        if let Err(e) = domain_model::validate(path, item.as_str(), &rule) {
                            err.extend(e);
                        }
                    }
                }
                    ...
                    quote! {
                        {
                            ...
                            for (i, item) in self.#field_name.iter().enumerate() {
                                let path = domain_model::Path::root().field(#field_name_str).index(i);
                                if let Err(e) = domain_model::validate(path, item, &rule) {
                                    err.extend(e);
                                }
                            }
                        }
